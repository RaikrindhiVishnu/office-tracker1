...
// ── CRUD helpers ─────────────────────────────────────────
const addItem = (col, data) => addDoc(collection(db, col), { ...data, createdAt: serverTimestamp() });
const updateItem = (col, id, data) => updateDoc(doc(db, col, id), { ...data, updatedAt: serverTimestamp() });
...
function Btn({ children, onClick, variant = "ghost", size = "sm", disabled = false, style = {}, icon }) {
  const variants = {
...
function Progress({ value, color = T.primary, h = 6 }) {
  return (
...
const statusProps = (s) => {
  const map = {
    "On Track": { color: T.success }, Completed: { color: T.success }, Active: { color: T.success },
    completed: { color: T.success }, converted: { color: T.success },
    "At Risk": { color: T.warning }, "On Leave": { color: T.warning }, pending: { color: T.warning },
    contacted: { color: T.cyan }, qualified: { color: T.purple }, proposal: { color: T.primary },
    Delayed: { color: T.danger }, new: { color: T.textMuted }, "In Progress": { color: T.primary },
  };
  const found = map[s];
  return found ? { ...found, label: s.charAt(0).toUpperCase() + s.slice(1) } : { color: T.textMuted, label: s || "Unknown" };
};

const priorityColor = (p) => ({ Critical: T.danger, High: T.warning, Medium: T.primary, Low: T.textMuted }[p] || T.textMuted);
  ...
  return (
    <Modal title="Confirm Delete" onClose={onCancel} width={400}>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        ...
        <Btn onClick={async () => { setDeleting(true); await onConfirm(); }} variant="danger" size="md" disabled={deleting}>{deleting ? "Deleting…" : "Delete"}</Btn>
      </div>
    </Modal>
  );
}
...
function ProjectModal({ project, employees, onClose }) {
  const [f, setF] = useState(project || { name: "", team: "", lead: "", progress: 0, budget: "", status: "On Track", priority: "Medium", deadline: "" });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const save = async () => {
    if (!f.name.trim()) return;
    setSaving(true);
    const payload = { ...f, budget: Number(f.budget) || 0, progress: Number(f.progress) || 0 };
  ...
  return (
    <Modal title={project ? "Edit Project" : "New Project"} onClose={onClose}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px" }}>
        ...
        <Field label="Team"><input value={f.team} onChange={e => set("team", e.target.value)} placeholder="e.g. Engineering" /></Field>
  ...
  {employeesFull.map(emp => (
    <option key={emp.id} value={emp.name}>
      {emp.name} ({emp.department})
        ...
        <Field label="Progress (%)"><input type="number" min={0} max={100} value={f.progress} onChange={e => set("progress", Number(e.target.value))} /></Field>
...
<Field label="Attendance (%)">
  <input type="number" value={f.attendance} onChange={e => set("attendance", e.target.value)} />
</Field>

<Field label="Tasks">
  <input type="number" value={f.tasks} onChange={e => set("tasks", e.target.value)} />
</Field>
      </div>
...
function EmployeeModal({ employee, onClose }) {
  const [f, setF] = useState(employee || { name: "", department: "", role: "", email: "", status: "Active", productivity: 85 });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const save = async () => {
    if (!f.name.trim()) return;
    setSaving(true);
    const payload = { ...f, productivity: Number(f.productivity) || 0 };
  ...
  return (
    <Modal title={employee ? "Edit Employee" : "Add Employee"} onClose={onClose}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px" }}>
        ...
        <Field label="Productivity (%)"><input type="number" min={0} max={100} value={f.productivity} onChange={e => set("productivity", Number(e.target.value))} /></Field>
      </div>
...
function TaskModal({ task, employees, onClose }) {
  const [f, setF] = useState(task || { title: "", assignedTo: "", priority: "Medium", status: "pending", dueDate: "", project: "" });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const save = async () => {
    if (!f.title.trim()) return;
    setSaving(true);
    if (f.id) await updateItem("tasks", f.id, f);
    else await addItem("tasks", f);
    onClose();
  };
  return (
  ...
  {employeesFull.map(emp => (
    <option key={emp.id} value={emp.name}>
      ...
      <FormActions onClose={onClose} onSave={save} saving={saving} saveLabel={task ? "Update" : "Create Task"} />
    </Modal>
  );
}

function SaleModal({ sale, onClose }) {
  const [f, setF] = useState(sale || { month: "", revenue: "", expenses: "", profit: "", target: "" });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const save = async () => {
    if (!f.month.trim()) return;
    setSaving(true);
    const payload = { month: f.month, revenue: Number(f.revenue) || 0, expenses: Number(f.expenses) || 0, profit: Number(f.profit) || 0, target: Number(f.target) || 0 };
  ...
  return (
    <Modal title={sale ? "Edit Sales Record" : "Add Sales Record"} onClose={onClose}>
      <Field label="Month (e.g. Jan, Feb)"><input value={f.month} onChange={e => set("month", e.target.value)} placeholder="Jan" /></Field>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px" }}>
        <Field label="Revenue ($)"><input type="number" value={f.revenue} onChange={e => set("revenue", e.target.value)} /></Field>
        <Field label="Expenses ($)"><input type="number" value={f.expenses} onChange={e => set("expenses", e.target.value)} /></Field>
        <Field label="Profit ($)"><input type="number" value={f.profit} onChange={e => set("profit", e.target.value)} /></Field>
...
function ExpenseModal({ onClose }) {
  ...
  return (
    <Modal title="Log Expense" onClose={onClose}>
      ...
      <Field label="Description"><textarea value={f.description} onChange={e => set("description", e.target.value)} style={{ height: 80, resize: "none" }} /></Field>
...
function AlertsPanel({ alerts, onClose }) {
  const typeColor = { warning: T.warning, danger: T.danger, info: T.primary, success: T.success };
  const typeIcon = { warning: "⚠️", danger: "🔴", info: "ℹ️", success: "✅" };
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontWeight: 800, fontSize: 14 }}>Alerts <span style={{ background: T.dangerLt, color: T.danger, padding: "2px 7px", borderRadius: 99, fontSize: 10, fontWeight: 700, marginLeft: 6 }}>{alerts.length}</span></span>
      ...
      {alerts.map((a, i) => (
        <div key={i} style={{ padding: "13px 16px", borderBottom: `1px solid ${T.border}`, background: (typeColor[a.type] || T.primary) + "08" }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            <span style={{ fontSize: 15, flexShrink: 0 }}>{typeIcon[a.type] || "ℹ️"}</span>
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, color: typeColor[a.type] || T.primary, marginBottom: 2 }}>{a.title}</p>
...
//  EMPTY STATE
// ══════════════════════════════════════════════════════════
function EmptyState({ icon = "📭", title = "No data yet", sub = "Add your first record to get started." }) {
  return (
...
export default function ExecutiveDashboard() {
  ...
  const { data: projects, loading: projLoading } = useCollection("projects");
  const { data: users, loading: empLoading } = useCollection("users");
  ...
  .map(u => ({
    ...
    // ✅ FIX HERE
    department: u.accountType || "General",
  ...
  const { data: customers, loading: custLoading } = useCollection("customers");
  const { data: leads, loading: leadsLoading } = useCollection("leads");
  const { data: expenses, loading: expLoading } = useCollection("expenses");
  const { data: alerts, loading: alertsLoading } = useCollection("alerts");
  const { data: payroll } = useCollection("payroll");
  const { data: assets } = useCollection("assets");
  ...
  const pendingPayments = customers.filter(c => c.status === "At Risk").reduce((s, c) => s + (c.value || 0), 0);
  ...
  const deptData = Object.entries(deptMap).map(([name, headcount]) => ({
    name, headcount, value: employees.length ? Math.round((headcount / employees.length) * 100) : 0,
  }));
  ...
  // ── Delete helper ────────────────────────────────────────
  const confirmDelete = (col, id, name) => {
    setConfirm({
  ...
  // ── Advance lead ─────────────────────────────────────────
  const advanceLead = async (l) => {
    const stages = ["new", "contacted", "qualified", "proposal", "converted"];
    const next = stages[stages.indexOf(l.status) + 1];
    if (next) await updateItem("leads", l.id, { status: next });
  };

  // ── Complete task ────────────────────────────────────────
  const completeTask = (t) => updateItem("tasks", t.id, { status: "completed" });
  const completeProject = (p) => updateItem("projects", p.id, { status: "Completed", progress: 100 });
  ...
  const globalLoading = salesLoading || projLoading || empLoading;
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {sideOpen && (
              <div>
                ...
                {/* <div style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>Executive Suite</div> */}
              </div>
            )}
          </div>
        </div>
        ...
        <header style={{
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 1 }}>
              ...
              {/* <span style={{ fontSize: 10, color: T.success, fontWeight: 700, letterSpacing: "0.1em", fontFamily: "'JetBrains Mono', monospace" }}>LIVE · FIREBASE</span> */}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ position: "relative" }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" style={{ width: 190, paddingLeft: "32px !important" }} />
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
            ...
            <div className="fade-up">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 14, marginBottom: 14 }}>
                <Card style={{ padding: 20 }}>
                  <SectionHeader title="Revenue vs Expenses" subtitle="Monthly trend" badge="Live" action={canAdd && <Btn onClick={() => setModal({ type: "sale" })} variant="primary" size="sm" icon="＋">Add Record</Btn>} />
                  {salesLoading ? <Skeleton h={220} /> : sales.length === 0 ? <EmptyState icon="📊" title="No sales data" sub="Add monthly records to see charts." /> : (
                    <ResponsiveContainer width="100%" height={220}>
                      <ComposedChart data={[...sales].sort((a, b) => (a.month || "").localeCompare(b.month || ""))}>
                        ...
                        {sales[0]?.target !== undefined && <Line type="monotone" dataKey="target" stroke={T.danger} strokeWidth={1.5} strokeDasharray="5 4" name="Target" dot={false} />}
                    ...
                    <>
                      <ResponsiveContainer width="100%" height={130}>
                        <PieChart>
                          ...
                          <Tooltip formatter={v => [`${v}%`, "Share"]} contentStyle={{ background: "#fff", border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 12 }} />
                        </PieChart>
                      </ResponsiveContainer>
                ...
                <Card style={{ padding: 20 }}>
                  ...
                  {leadsLoading ? <Skeleton h={200} /> : leads.length === 0 ? <EmptyState icon="🎯" title="No leads" sub="Add leads to track your pipeline." /> : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {salesFunnel.filter(s => s.value > 0 || true).map((stage, i) => {
                        ...
                        return (
                          <div key={stage.name}>
                            {drop !== null && drop > 0 && <div style={{ fontSize: 10, color: T.danger, textAlign: "right", fontFamily: "'JetBrains Mono', monospace" }}>↓ {drop}% drop</div>}
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {tab === "finance" && (
            <div className="fade-up">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                <Card style={{ padding: 20 }}>
                  <SectionHeader title="Expense Breakdown" subtitle="From expense log" action={canAdd && <Btn onClick={() => setModal({ type: "expense" })} variant="primary" size="sm" icon="＋">Log Expense</Btn>} />
                    ...
                    <>
                      <ResponsiveContainer width="100%" height={160}>
                        <PieChart>
                          ...
                          <Tooltip formatter={v => [fmtMoney(v), "Amount"]} contentStyle={{ background: "#fff", border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 12 }} />
                        </PieChart>
                      </ResponsiveContainer>
                ...
                <Card style={{ padding: 20 }}>
                  ...
                  {salesLoading ? <Skeleton h={260} /> : sales.length === 0 ? <EmptyState icon="📊" title="No sales data" /> : (
                    <ResponsiveContainer width="100%" height={260}>
                      <BarChart data={[...sales].sort((a, b) => (a.month || "").localeCompare(b.month || "")).map(r => ({ month: r.month, margin: r.revenue ? +((r.profit / r.revenue) * 100).toFixed(1) : 0 }))}>
                        <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
                        <XAxis dataKey="month" tick={{ fill: T.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} />
              ...
              {/* Expense log table */}
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Expense Log" subtitle={`${expenses.length} entries`} action={<Btn onClick={() => exportCSV(expenses, "expenses.csv")} variant="outline" size="sm" icon="⬇">Export</Btn>} />
                {expLoading ? <Skeleton h={120} /> : expenses.length === 0 ? <EmptyState icon="🧾" title="No expenses yet" /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {expenses.filter(e => !search || e.category?.toLowerCase().includes(search.toLowerCase()) || e.description?.toLowerCase().includes(search.toLowerCase())).map((e, idx) => (
          ...
          {tab === "analytics" && (
            <div className="fade-up">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  ...
                  {salesLoading ? <Skeleton h={240} /> : sales.length === 0 ? <EmptyState icon="📊" title="No sales data" /> : (
                    <ResponsiveContainer width="100%" height={240}>
                      <AreaChart data={[...sales].sort((a, b) => (a.month || "").localeCompare(b.month || ""))}>
                        <defs>
                          <linearGradient id="rA" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.primary} stopOpacity={0.13} /><stop offset="95%" stopColor={T.primary} stopOpacity={0} /></linearGradient>
                          <linearGradient id="eA" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.danger} stopOpacity={0.11} /><stop offset="95%" stopColor={T.danger} stopOpacity={0} /></linearGradient>
                        </defs>
                        <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
                        <XAxis dataKey="month" tick={{ fill: T.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} />
                        <YAxis tickFormatter={fmtShort} tick={{ fill: T.textMuted, fontSize: 10 }} axisLine={false} tickLine={false} />
                ...
                <Card style={{ padding: 20 }}>
                  <SectionHeader title="Sales Records" subtitle="Edit / manage monthly records" action={canAdd && <Btn onClick={() => setModal({ type: "sale" })} variant="primary" size="sm" icon="＋">Add</Btn>} />
                  ...
                  {projLoading ? <Skeleton h={200} /> : projects.length === 0 ? <EmptyState icon="🚀" title="No projects" /> : (() => {
                    ...
                    return (
                      <ResponsiveContainer width="100%" height={200}>
                        <PieChart>
                          <Pie data={pieData} cx="50%" cy="50%" outerRadius={70} paddingAngle={3} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
          ...
          {tab === "projects" && (
            <div className="fade-up">
              ...
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Projects" subtitle={`${activeProjects} active`} action={
                  <div style={{ display: "flex", gap: 8 }}>
                    {canAdd && <Btn onClick={() => setModal({ type: "project" })} variant="primary" size="sm" icon="＋">New Project</Btn>}
                    <Btn onClick={() => exportCSV(projects, "projects.csv")} variant="outline" size="sm" icon="⬇">CSV</Btn>
                  </div>
                } />
                {projLoading ? <Skeleton h={200} /> : projects.length === 0 ? <EmptyState icon="🚀" title="No projects yet" sub="Create your first project." /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {projects.filter(p => !search || p.name?.toLowerCase().includes(search.toLowerCase()) || p.lead?.toLowerCase().includes(search.toLowerCase())).map((p, idx) => {
                          ...
                          return (
                            <tr key={p.id || idx} className="hover-row" style={{ borderBottom: `1px solid ${T.border}` }}>
                              <td style={{ padding: "13px 12px" }}>
                                ...
                                <div style={{ fontSize: 10, color: T.textMuted, marginTop: 1 }}>Due: {p.deadline || "—"}</div>
                              </td>
                              <td style={{ padding: "13px 12px", color: T.textSub, fontSize: 12 }}>{p.lead || "—"}</td>
                              <td style={{ padding: "13px 12px", color: T.textMuted, fontSize: 12 }}>{p.team || "—"}</td>
                              ...
                              <td style={{ padding: "13px 12px" }}><Badge color={priorityColor(p.priority)}>{p.priority || "—"}</Badge></td>
                              <td style={{ padding: "13px 12px" }}>
          ...
          {tab === "employees" && (
            <div className="fade-up">
              ...
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Employees" subtitle={`${employees.length} total`} action={
                  <div style={{ display: "flex", gap: 8 }}>
                    {canAdd && <Btn onClick={() => setModal({ type: "employee" })} variant="primary" size="sm" icon="＋">Add Employee</Btn>}
                    <Btn onClick={() => exportCSV(employees, "employees.csv")} variant="outline" size="sm" icon="⬇">CSV</Btn>
                  </div>
                } />
                {empLoading ? <Skeleton h={200} /> : employees.length === 0 ? <EmptyState icon="👥" title="No employees yet" /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {employees.filter(e => !search || e.name?.toLowerCase().includes(search.toLowerCase()) || e.department?.toLowerCase().includes(search.toLowerCase())).map((e, idx) => {
                          const sp = statusProps(e.status);
                          const pc = e.productivity >= 90 ? T.success : e.productivity >= 70 ? T.warning : T.danger;
                          return (
                            <tr key={e.id || idx} className="hover-row" style={{ borderBottom: `1px solid ${T.border}` }}>
                              <td style={{ padding: "13px 12px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {tab === "tasks" && (
            <div className="fade-up">
              ...
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Tasks" subtitle="All assignments" action={canAdd && <Btn onClick={() => setModal({ type: "task" })} variant="primary" size="sm" icon="＋">Create Task</Btn>} />
                {tasksLoading ? <Skeleton h={200} /> : tasks.length === 0 ? <EmptyState icon="✅" title="No tasks yet" /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {tasks.filter(t => !search || t.title?.toLowerCase().includes(search.toLowerCase()) || t.assignedTo?.toLowerCase().includes(search.toLowerCase())).map((t, idx) => {
                          const sp = statusProps(t.status);
          ...
          {tab === "customers" && (
            <div className="fade-up">
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Customers" subtitle={`${customers.length} total`} action={
                  <div style={{ display: "flex", gap: 8 }}>
                    {canAdd && <Btn onClick={() => setModal({ type: "customer" })} variant="primary" size="sm" icon="＋">Add Customer</Btn>}
                    <Btn onClick={() => exportCSV(customers, "customers.csv")} variant="outline" size="sm" icon="⬇">Export</Btn>
                  </div>
                } />
                {custLoading ? <Skeleton h={200} /> : customers.length === 0 ? <EmptyState icon="🤝" title="No customers yet" /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {customers.filter(c => !search || c.name?.toLowerCase().includes(search.toLowerCase())).map((c, idx) => {
                          const sp = statusProps(c.status);
                          return (
                            <tr key={c.id || idx} className="hover-row" style={{ borderBottom: `1px solid ${T.border}` }}>
                              ...
                              <td style={{ padding: "13px 12px" }}>
                                <div style={{ display: "flex", gap: 4 }}>
                                  {canAdd && <Btn onClick={async () => { await addItem("leads", { name: c.name, email: c.email, status: "contacted", value: c.value || 0, source: c.source || "" }); }} variant="success" size="sm">→ Lead</Btn>}
          ...
          {tab === "leads" && (
            <div className="fade-up">
              ...
              <Card style={{ padding: 20 }}>
                <SectionHeader title="Sales Pipeline" subtitle={`${leads.length} leads · ${conversionRate}% conversion`} action={canAdd && <Btn onClick={() => setModal({ type: "lead" })} variant="primary" size="sm" icon="＋">Add Lead</Btn>} />
                {leadsLoading ? <Skeleton h={200} /> : leads.length === 0 ? <EmptyState icon="🎯" title="No leads yet" /> : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {leads.filter(l => !search || l.name?.toLowerCase().includes(search.toLowerCase())).map((l, idx) => {
                          const sp = statusProps(l.status);
                          return (
          ...
          {tab === "expenses" && (
            <div className="fade-up">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                <Card style={{ padding: 20 }}>
                  <SectionHeader title="By Category" subtitle="Expense distribution" action={canAdd && <Btn onClick={() => setModal({ type: "expense" })} variant="primary" size="sm" icon="＋">Log</Btn>} />
                ...
                <Card style={{ padding: 20 }}>
                  <SectionHeader title="All Expenses" subtitle="Full log" action={<Btn onClick={() => exportCSV(expenses, "expenses.csv")} variant="outline" size="sm" icon="⬇">Export</Btn>} />
          ...
          {/* <div style={{ textAlign: "center", color: T.textMuted, fontSize: 10, marginTop: 28, paddingBottom: 12, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.08em" }}>
            APEX ERP · EXECUTIVE SUITE · FIREBASE: office-tracker-33067 · ROLE: {role.toUpperCase()} ·{new Date().toLocaleDateString("en-GB")}
          </div> */}
        </main>
      </div>

      {/* ══ MODALS ════════════════════════════════════════════ */}
      {modal?.type === "project" && <ProjectModal project={modal.data} employees={employees} onClose={() => setModal(null)} />}
