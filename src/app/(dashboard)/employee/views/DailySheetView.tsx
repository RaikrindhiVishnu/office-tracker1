...
export default function DailySheetView() {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#f4f6f9] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <div className="lg:hidden mx-2 sm:mx-4 mb-6 space-y-4">
        {sortedDates.length === 0 ? (
          ...
          sortedDates.map((dateStr, idx) => {
            ...
            return (
              <div key={dateStr} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 relative">
                 <div className="flex justify-between items-center mb-3">
                   <span className="font-bold text-slate-700">{formatDate(dateStr)} <span className="text-xs font-normal text-slate-500 ml-1">{new Date(dateStr + "T00:00:00").toLocaleDateString("en-US", { weekday: "short" })}</span></span>
                   
                   {!hasEntry && isWeekend && <span className="text-[10px] font-bold uppercase text-slate-500 bg-slate-100 px-2 py-1 rounded">Weekend</span>}
                   {!hasEntry && !isWeekend && checkedIn && <span className="text-[10px] font-bold uppercase text-amber-700 bg-amber-100 px-2 py-1 rounded">Not Filled</span>}
                   {!hasEntry && !isWeekend && !checkedIn && pastDeadline && <span className="text-[10px] font-bold uppercase text-red-700 bg-red-100 px-2 py-1 rounded">Absent</span>}
                   {!hasEntry && !isWeekend && !checkedIn && !pastDeadline && <span className="text-[10px] font-bold uppercase text-slate-500 bg-slate-100 px-2 py-1 rounded">Pending</span>}
                   
                   {hasEntry && isHoliday && <span className="text-[10px] font-bold uppercase text-amber-700 bg-amber-100 px-2 py-1 rounded">Holiday</span>}
                   {hasEntry && !isHoliday && dayEntries.some(e => e.isDraft) && <span className="text-[10px] font-bold uppercase text-slate-500 bg-slate-100 px-2 py-1 rounded">Draft</span>}
                   {hasEntry && !isHoliday && !dayEntries.some(e => e.isDraft) && <span className="text-[10px] font-bold uppercase text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-1 rounded">Submitted</span>}
                 </div>
                 ...
                 {!isWeekend && (
                   <div className="grid grid-cols-3 gap-2 mb-3 bg-slate-50 p-2 rounded-lg border border-slate-100 text-center">
                     <div>
                       ...
                       <div className="text-xs font-semibold text-slate-700">{att?.in || "—"}</div>
                     </div>
                     <div className="border-x border-slate-200">
                       <div className="text-[9px] font-bold text-slate-400 uppercase">Out</div>
                       <div className="text-xs font-semibold text-slate-700">{att?.out || "—"}</div>
                     </div>
                     <div>
                       <div className="text-[9px] font-bold text-slate-400 uppercase">Sys Hrs</div>
                       <div className="text-xs font-semibold text-indigo-600">{att?.sys || "—"}</div>
                     </div>
                   </div>
                 )}
                 ...
                 {hasEntry && !isHoliday && (
                   <div className="mb-3 space-y-2">
                     {normalizedTasks.filter(t => t.dateStr === dateStr).map((t, i) => (
                       <div key={t.taskId || i} className="text-xs text-slate-600 border border-slate-100 rounded-lg p-2.5 bg-white shadow-sm relative pr-10">
                         <div className="absolute top-2 right-2 flex gap-1">
                            {!isLocked && (
                                <button onClick={(e) => { e.stopPropagation(); handleEditTask(docId, t); }} className="text-indigo-400 hover:text-indigo-600">
                                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </button>
                            )}
                            {!isLocked && (
                                <button onClick={(e) => { e.stopPropagation(); handleDeleteTask(t.entryId, t.taskId); }} className="text-rose-400 hover:text-rose-600">
                 ...
                 {!isWeekend && !isHoliday && !isLocked && (
                   <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100">
                     <button
                       onClick={() => { resetForm(); setEntryDate(dateStr); setIsModalOpen(true); }}
                       className="flex-1 py-1.5 text-xs font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded transition text-center"
                     >
                       + Add Task
                     </button>
                     {hasEntry && (
                       <button
                         ...
                         className="p-1.5 text-rose-400 hover:text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded transition"
                       >
                         <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                         </svg>
      ...
      {/* ── TABLE (DESKTOP ONLY) ── */}
      <div className="hidden lg:block mx-2 sm:mx-6 mb-6 bg-white rounded-b-xl shadow-sm border border-slate-200 overflow-hidden">
