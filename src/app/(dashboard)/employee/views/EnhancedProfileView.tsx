...
export default function EnhancedProfileView() {
  ...
  const calculateCompletion = () => {
    const importantKeys = [
      "name", "email", "phone", "dateOfBirth", "gender", "bloodGroup", 
      "address", "employeeId", "department", "emergencyContactName", 
      "emergencyContactPhone", "panNumber", "aadharNumber", "skills", "education"
    ];
    let filled = 0;
    importantKeys.forEach(k => {
      if (form[k] && String(form[k]).trim() !== "") filled++;
    });
    return Math.round((filled / importantKeys.length) * 100);
  };
    ...
    <div className="min-h-screen bg-slate-50/50 p-2 sm:p-4 pt-1">
      <div className="max-w-7xl mx-auto space-y-3">
          ...
          <div className="bg-gradient-to-r from-slate-950 via-slate-800 to-slate-950 px-8 py-8 flex flex-col sm:flex-row items-center sm:justify-between gap-6 relative">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 w-full sm:w-auto text-center sm:text-left">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm font-medium text-slate-300 mb-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 max-w-[200px] h-2.5 bg-slate-800 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-1000 ${completionPct === 100 ? "bg-emerald-500" : "bg-indigo-500"}`} style={{ width: `${completionPct}%` }} />
                  </div>
                  <span className="text-xs font-bold text-slate-300">{completionPct}% Profile Completed</span>
                </div>
            ...
            {/* Action Buttons */}
            <div className="flex w-full sm:w-auto gap-3 mt-4 sm:mt-0 z-10 relative flex-wrap justify-end">
              {isEditing ? (
                <>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <InfoSection title="Skills & Education" icon="🎓" isEditing={isEditing}>
            <div className="grid grid-cols-1 gap-3">
              <TextAreaField label="Core Skills (comma separated)" value={form.skills} editing={isEditing} onChange={setF("skills")} />
              {form.skills && !isEditing && (
                <div className="flex flex-wrap gap-2 mt-1">
                  {form.skills.split(",").map((skill: string, i: number) => (
                    skill.trim() && <span key={i} className="px-2.5 py-1 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-lg text-xs font-bold">{skill.trim()}</span>
                  ))}
                </div>
              )}
              <TextAreaField label="Education & Certifications" value={form.education} editing={isEditing} onChange={setF("education")} />
            </div>
          </InfoSection>
