...
═══════════════════════════════════════════════════════════════ */
const Ic = {
  Mail:     ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="2,4 12,13 22,4"/></svg>,
  ...
  Brief:    ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="2" y="8" width="20" height="13" rx="2"/><path d="M16 8V6a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>,
  Home:     ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
  Bank:     ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>,
  Alert:    ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>,
  ...
  Location: ()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
};
...
function SectionCard({ title, iconBg, icon, children, onEdit, editing, onSave, onCancel, saving, id }:{
  title:string; iconBg:string; icon:React.ReactNode; children:React.ReactNode;
  onEdit:()=>void; editing:boolean; onSave:()=>void; onCancel:()=>void; saving?:boolean; id?:string;
}) {
  return (
    <div id={id} className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-4 overflow-hidden">
  ...
  return (
    <div className="min-h-screen bg-[#EEF0F5]" style={{fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}>
      <div className="max-w-6xl mx-auto px-4 py-2 flex gap-4 items-start" style={{minHeight:"calc(100vh - 2.5rem)"}}>
        ...
        {/* ══════════════════════════════════════════════════════
            LEFT SIDEBAR
            — sticky + fixed height + internal scroll so it
        ...
        >
          ...
          >
            {/* Cover */}
            <div className="relative shrink-0" style={{height:56, background:"linear-gradient(135deg,#1a2340 0%,#2d3a5e 60%,#3d4f7c 100%)"}}>
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 z-10">
                <div
                  {...getRootProps()}
                  className="relative rounded-2xl border-[3px] border-white shadow-lg overflow-hidden cursor-pointer group"
                  style={{width:56, height:56}}
                >
                  <input {...getInputProps()}/>
                  {photoPreview
                    ? <img src={photoPreview} alt="avatar" className="w-full h-full object-cover"/>
                    : <div className="w-full h-full flex items-center justify-center text-white text-base font-bold" style={{background:"linear-gradient(135deg,#7C3AED,#4F46E5)"}}>{initials}</div>
                  }
            ...
            {/* Name / badge — fixed height, no scroll */}
            <div className="shrink-0 pt-9 pb-3 px-4 text-center border-b border-gray-100">
              <div className="flex items-center justify-center gap-1 flex-wrap">
                <h2 className="text-[13px] font-bold text-gray-900 leading-tight">{localData.name||"Employee"}</h2>
                {localData.employeeId && (
                  <button onClick={copyId} title="Copy ID"
                    className="flex items-center gap-0.5 text-[10px] font-semibold text-gray-400 hover:text-violet-500 transition px-1 py-0.5 rounded hover:bg-violet-50">
                    {localData.employeeId} {copied ? "✓" : <Ic.Copy/>}
                  </button>
                )}
              </div>
              {localData.designation && (
                <span className="inline-block mt-1.5 text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-violet-50 text-violet-600 tracking-wide">
                  {localData.designation}
                </span>
              )}
              ...
              {profilePhoto && (
                <button onClick={savePhoto} disabled={isSaving("photo")}
                  className="mt-2 w-full text-[11px] font-semibold py-1 rounded-lg bg-violet-600 hover:bg-violet-700 text-white transition disabled:bg-gray-300">
                  {isSaving("photo") ? "Saving…" : "Save New Photo"}
                </button>
              )}
            </div>

            {/* Basic info list — scrollable */}
            <div className="overflow-y-auto px-4 py-3" style={{minHeight:0}}>
              <p className="text-[9px] font-extrabold tracking-[0.15em] uppercase text-gray-400 mb-2">Basic Information</p>
              <div className="space-y-0">
                {[
                  ...
                  <div key={label} className={`flex items-start gap-2 py-2 ${i < arr.length - 1 ? "border-b border-gray-50" : ""}`}>
                    <span className="mt-0.5 text-gray-400 shrink-0">{icon}</span>
                    <div className="min-w-0">
                      <p className="text-[9px] font-bold tracking-wide text-gray-400 uppercase leading-tight">{label}</p>
        ...
        >
          ...
          {/* Tab bar */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-4 px-3 py-2 flex items-center gap-1 overflow-x-auto sticky top-0 z-10">
            {TABS.map(tab=>(
              <button
                key={tab}
                onClick={()=>setActiveTab(tab)}
                className={[
                  "shrink-0 px-3 py-2 rounded-xl text-sm font-semibold transition-all whitespace-nowrap",
                  activeTab===tab
