  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-md p-4 w-full max-w-sm">
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100">
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          <span className="w-3 h-3 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 inline-block"/>Selected
        </div>
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <div className="flex items-center gap-2">
              ...
              {getPageNumbers(currentPage,totalPages).map((page,i)=>(
                <button key={i} onClick={()=>typeof page==="number"&&setCurrentPage(page)} disabled={page==="..."}
                  className={`min-w-[2.5rem] px-3 py-2 rounded-lg font-medium transition-all ${page===currentPage?"bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg":page==="..."?"text-slate-400 cursor-default":"bg-white border-2 border-slate-200 text-slate-700 hover:bg-slate-50"}`}>
                  {page}
                </button>
              ))}
                      ...
                      <div className="flex items-center justify-center gap-2 pt-2">
                        ...
                        {getPageNumbers(historyPage,historyTotalPages).map((p,i)=>(
                          <button key={i} onClick={()=>typeof p==="number"&&setHistoryPage(p)} disabled={p==="..."}
                            className={`min-w-[2.25rem] px-3 py-1.5 rounded-lg text-sm font-semibold transition-all ${p===historyPage?"bg-indigo-600 text-white shadow":p==="..."?"text-slate-400 cursor-default":"border-2 border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                            {p}
                          </button>
                        ))}
