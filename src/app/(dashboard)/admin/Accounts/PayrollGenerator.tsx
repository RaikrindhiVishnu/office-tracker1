...
export default function PayrollGenerator() {
  ...
  const loadEmployees = async () => {
    ...
    for (const docSnap of snap.docs) {
      ...
      const payslipSnap = await getDoc(
        doc(db, "payslips", `${uid}_${monthKey}`)
      );
      list.push({
        uid,
        name: data.name,
  ...
  const handleGenerateAll = async () => {
    ...
    if (pending.length === 0) {
      alert("✅ All payslips already generated for this month.");
      return;
    }
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      if (ok) {
        ...
        failNames.push(emp.name || emp.uid);
      }
    ...
    if (failNames.length === 0) {
      ...
      alert(
        `✅ ${successCount} payslips generated.\n❌ Failed (no salary structure):\n${failNames.join(", ")}`
      );
    }
  };

  /* ================= DOWNLOAD ================= */
  const downloadPayslip = async (uid: string) => {
    const { default: jsPDF } = await import("jspdf");

    // ── Load logo from /public/logo.svg ──
    ...
    // ── LOGO ─────────────────────────────────────────
    if (logoDataUrl) {
      pdf.addImage(logoDataUrl, "PNG", 18, 10, 145, 100);
    }
    ...
    const val = (text: string, x: number, y: number) => {
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(10);
      pdf.setTextColor(0, 0, 0);
    ...
    lbl("Employee Name:",   28, TOP_Y);
    lbl("Emp ID:",          28, TOP_Y + LINE_H);
    lbl("Designation:",     28, TOP_Y + LINE_H * 2);
    lbl("Date of Joining:", 28, TOP_Y + LINE_H * 3);

    val(user?.name || "-", 145, TOP_Y);
    val(empId,              145, TOP_Y + LINE_H);
    val(designation,        145, TOP_Y + LINE_H * 2);
    val(doj,                145, TOP_Y + LINE_H * 3);

    lbl("Total Days In Month:", 318, TOP_Y);
    lbl("LOP",                  318, TOP_Y + LINE_H);
    lbl("Actual Paid Days",     318, TOP_Y + LINE_H * 2);
    lbl("Bank Account No.",     318, TOP_Y + LINE_H * 3);

    val(String(totalDays),  470, TOP_Y);
    val(String(lop),        470, TOP_Y + LINE_H);
    val(String(paidDays),   470, TOP_Y + LINE_H * 2);
    val(bankAccount,        470, TOP_Y + LINE_H * 3);
    ...
    const tableRows: [string, string, string, string, boolean, boolean][] = [
      ["Earnigs",            "Amount",              "Deduction",        "Amount",                true,  false],
    ...
    tableRows.forEach(([el, ev, dl, dv, isHeader, isTotal], i) => {
      ...
      [el, ev, dl, dv].forEach((text, ci) => {
        ...
        if (isHeader || isTotal) {
          ...
          pdf.rect(x, ry, w, ROW_H, "F");
        }

        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(0.5);
        pdf.rect(x, ry, w, ROW_H, "S");
        ...
        if (isHeader) {
          pdf.text(text, x + w / 2, ty, { align: "center" });
        } else if ((ci === 1 || ci === 3) && text) {
          pdf.text(text, x + w - 6, ty, { align: "right" });
        } else {
          pdf.text(text, x + 6, ty);
        }
      });
    });
    ...
    pdf.text(
      "This is system Generated document. doesn't need a signature",
      W / 2, NET_Y + 55, { align: "center" }
    );
    pdf.text(
      "For Verification reach hr@techgyinnovations.com",
      W / 2, NET_Y + 69, { align: "center" }
    );
    ...
    <div className="bg-white p-8 rounded-2xl shadow-lg space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          disabled={loadingAll || pendingCount === 0}
              ...
              employees.map((emp) => (
                <tr key={emp.uid} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-3 text-center">
                    {emp.generated ? (
                      <button
                        onClick={() => downloadPayslip(emp.uid)}
