...
const NAV_ITEMS:{id:Tab;label:string;icon:string}[] = [
  ...
  {id:"anniversaries",label:"Work Anniversary", icon:"🎉"},
...
export default function GreetingsAdmin() {
  ...
  const saveAch=async()=>{
    ...
    try{const payload={...achForm,employeeName:achForm.employeeName.trim(),employeeEmail:achForm.employeeEmail.trim().toLowerCase(),title:achForm.title.trim(),description:achForm.description.trim(),updatedAt:serverTimestamp()};
      ...
      setAchModal(null);setAchForm(emptyAch);setFormErr({});showToast(achModal==="new"?"🏆 Achievement added!":"✏️ Updated!");}
  ...
  const saveJoin=async()=>{
    ...
    try{const payload={...joinForm,name:joinForm.name.trim(),email:joinForm.email.trim().toLowerCase(),department:joinForm.department.trim()||null,designation:joinForm.designation.trim()||null,updatedAt:serverTimestamp()};
      ...
      setJoinModal(null);setJoinForm(emptyJoin);setFormErr({});showToast(joinModal==="new"?"👋 New joiner added!":"✏️ Updated!");}
    catch{showToast("Save failed",false);}finally{setSaving(false);}
  };
  ...
  const saveFest=async()=>{
    ...
    try{const payload={...festForm,title:festForm.title.trim(),emailSubject:festForm.emailSubject.trim(),emailMessage:festForm.emailMessage.trim(),updatedAt:serverTimestamp()};
      ...
      setFestModal(null);setFestForm(emptyFest);setFormErr({});showToast(festModal==="new"?"🪔 Festival added!":"✏️ Updated!");}
    catch{showToast("Save failed",false);}finally{setSaving(false);}
  };
  ...
  const saveEvent=async()=>{
    ...
    try{const payload={...eventForm,title:eventForm.title.trim(),description:eventForm.description.trim(),location:eventForm.location?.trim()||null,rsvpLink:eventForm.rsvpLink?.trim()||null,updatedAt:serverTimestamp()};
      ...
      setEventModal(null);setEventForm(emptyEvent);setFormErr({});showToast(eventModal==="new"?"📅 Event added!":"✏️ Updated!");}
    catch{showToast("Save failed",false);}finally{setSaving(false);}
  };
  ...
  const notifications = [
    ...
    ...upcomingFests.filter(f=>daysUntil(f.festivalDate)<=3).map(f=>({icon:f.bannerEmoji||"🪔",msg:`${f.title} in ${daysUntil(f.festivalDate)}d`,color:"#b45309",action:()=>{setTab("festivals");setShowNotifs(false);}})),
  ];
    ...
    <>
            ...
            <div className="gr-tb-right">
              ...
              {showNotifs&&(
                <div className="gr-panel" style={{right:80}}>
                  <div className="gr-panel-hd">
                    <div className="gr-panel-title">🔔 Notifications <span style={{fontSize:10,fontWeight:600,color:"var(--text3)",marginLeft:4}}>{notifications.length} new</span></div>
                    <button className="gr-panel-close" onClick={()=>setShowNotifs(false)}>×</button>
                  </div>
                  <div className="gr-panel-body">
                    {notifications.length===0
                      ?<div className="gr-panel-empty">🎉 All caught up! No new notifications.</div>
              ...
              {showEmails&&(
                <div className="gr-panel" style={{right:40}}>
                  <div className="gr-panel-hd">
                    <div className="gr-panel-title">📧 Recent Emails <span style={{fontSize:10,fontWeight:600,color:"var(--text3)",marginLeft:4}}>{successCount} sent · {failCount} failed</span></div>
                    <button className="gr-panel-close" onClick={()=>setShowEmails(false)}>×</button>
                  </div>
                  <div className="gr-panel-body">
                    {recentEmails.length===0
                      ?<div className="gr-panel-empty">📭 No emails sent yet.</div>
                      :recentEmails.map(l=>(
          ...
          <div className="gr-scroll">
            ...
            {tab==="dashboard"&&(
              <>
                <div className="gr-stats">
                  <StatCard val={recipientCount} lbl="Total Employees" icon="👥" bg="#eff6ff" color="#1d4ed8" badge="Active" badgeBg="#dbeafe" barPct={100}/>
                  <StatCard val={nextBdayList.filter(e=>e.daysLeft===0).length||0} lbl="Birthdays Today" icon="🎂" bg="#fdf4ff" color="#7c3aed" badge={nextBdayList.filter(e=>e.daysLeft===0).length>0?"🎉 Today!":nextBday?"Next: "+nextBday.daysLeft+"d":undefined} badgeBg="#f3e8ff" barPct={nextBdayList.filter(e=>e.daysLeft===0).length>0?100:0}/>
                  <StatCard val={todayAnnivsrs.length} lbl="Anniversaries Today" icon="🎉" bg="#fffbeb" color="#d97706" badge={todayAnnivsrs.length>0?"Today!":undefined} badgeBg="#fef3c7" barPct={todayAnnivsrs.length>0?100:0}/>
                  <StatCard val={successCount} lbl="Emails Sent" icon="📧" bg="#f0fdf4" color="#059669" badge={failCount+" failed"} badgeBg={failCount>0?"#fff1f2":"#f0fdf4"} barPct={logs.length>0?Math.round((successCount/logs.length)*100):0}/>
                </div>
                ...
                <div className="gr-tiles">
                  {[
                    {icon:"🎂",label:"Birthday Wish",    sub:"Individual",bg:"#fdf4ff",action:()=>setTab("birthdays")},
                    {icon:"🎉",label:"Work Anniversary", sub:"Individual",bg:"#fffbeb",action:()=>setTab("anniversaries")},
                    {icon:"🏆",label:"Achievement Award",sub:"Individual",bg:"#f5f3ff",action:()=>setTab("achievements")},
                    ...
                    <div key={t.label} className="gr-tile" onClick={t.action}>
                      <div className="gr-tile-ico" style={{background:t.bg}}>{t.icon}</div>
                ...
                <div className="gr-grid2">
                  ...
                  <div className="gr-card">
                    <div className="gr-card-hd">
                      <div className="gr-card-title">🎂 Next Upcoming Birthday</div>
                      {sameDayBdays.length>1&&<span className="gr-b gr-b-purple">+{sameDayBdays.length} same day</span>}
                    </div>
                    {!nextBday ? (
                      <div className="gr-empty"><div className="gr-empty-ico">🌸</div>No employees with birthdays yet</div>
                    ) : (
                  ...
                  <div className="gr-card gr-grid-full">
                    ...
                    <div style={{padding:"14px 16px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:8}}>
                      ...
                      {logs.length===0&&<div className="gr-empty" style={{gridColumn:"1/-1"}}><div className="gr-empty-ico">📭</div>No emails sent yet</div>}
                    </div>
                  </div>
                </div>
              </>
            ...
            {tab==="achievements"&&(
              <>
                ...
                <div className="gr-type-banner individual"><span>🏆</span><span><strong>Individual Email</strong> — Achievement recognition sent to the awarded employee only.</span></div>
                <div className="gr-card">
                  <div className="gr-tw">
                    <table className="gr-table">
                    ...
                    {achievements.length===0&&<div className="gr-empty"><div className="gr-empty-ico">🏆</div>No achievements logged yet.</div>}
                  </div>
                </div>
              </>
            )}
            ...
            {tab==="welcome"&&(
              <>
                ...
                <div className="gr-type-banner individual"><span>👋</span><span><strong>Individual Email</strong> — Welcome emails sent to the new employee only.</span></div>
                <div className="gr-card">
                  <div className="gr-tw">
                    <table className="gr-table">
                    ...
                    {newJoiners.length===0&&<div className="gr-empty"><div className="gr-empty-ico">👋</div>No new joiners added yet.</div>}
                  </div>
                </div>
              </>
            )}
            ...
            {tab==="festivals"&&(
              <>
                ...
                <div className="gr-type-banner broadcast"><span>📢</span><span><strong>Broadcast</strong> — Festival greetings sent to <strong>all {recipientCount} employees</strong> at once.</span></div>
                <div className="gr-card">
                  <div className="gr-tw">
                    <table className="gr-table">
                    ...
                    {festivals.length===0&&<div className="gr-empty"><div className="gr-empty-ico">🪔</div>No festivals added yet.</div>}
                  </div>
                </div>
              </>
            )}
            ...
            {tab==="events"&&(
              <>
                ...
                <div className="gr-type-banner broadcast"><span>📢</span><span><strong>Broadcast</strong> — Announcements & reminders sent to <strong>all {recipientCount} employees</strong>.</span></div>
                <div className="gr-card">
                  <div className="gr-tw">
                    <table className="gr-table">
                        ...
                        {events.slice().sort((a,b)=>a.eventDate.localeCompare(b.eventDate)).map(ev=>(
                          <tr key={ev.id}>
                            ...
                            <td><div style={{display:"flex",gap:4}}>
                              ...
                              <button className="gr-btn gr-btn-ghost gr-btn-sm" onClick={()=>{setEventModal(ev);setEventForm({title:ev.title,description:ev.description,eventDate:ev.eventDate,location:ev.location||"",color:ev.color||"#193677",sendAnnouncementEmail:ev.sendAnnouncementEmail,reminderDaysBefore:ev.reminderDaysBefore,rsvpLink:ev.rsvpLink||""});setFormErr({});}}>✏️</button>
                              <button className="gr-btn gr-btn-danger gr-btn-sm" disabled={deleting===ev.id} onClick={()=>deleteEvent(ev.id,ev.title)}>{deleting===ev.id?"…":"🗑️"}</button>
                            </div></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    {events.length===0&&<div className="gr-empty"><div className="gr-empty-ico">📅</div>No events added yet.</div>}
                  </div>
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="gr-card">
                  <div className="gr-tw">
                      ...
                      <tbody>
                        {filteredLogs.map(l=>(
                          <tr key={l.id}>
                            ...
                            <td><span className={"gr-b "+(l.sentBy==="admin"?"gr-b-blue":"gr-b-teal")}>{l.sentBy==="admin"?"👤":"🤖"} {l.sentBy}</span></td>
                    ...
                    {filteredLogs.length===0&&<div className="gr-empty"><div className="gr-empty-ico">📭</div>No history found</div>}
                  </div>
                </div>
              </>
            )}
      ...
      {achModal&&(
        <div className="gr-mbk" onClick={e=>e.target===e.currentTarget&&setAchModal(null)}>
          <div className="gr-modal">
            <div className="gr-mtitle">🏆 {achModal==="new"?"Add Achievement":"Edit Achievement"}</div>
      ...
      {joinModal&&(
        <div className="gr-mbk" onClick={e=>e.target===e.currentTarget&&setJoinModal(null)}>
          <div className="gr-modal">
            <div className="gr-mtitle">👋 {joinModal==="new"?"Add New Joiner":"Edit New Joiner"}</div>
            <div className="gr-2col">
      ...
      {eventModal&&(
        <div className="gr-mbk" onClick={e=>e.target===e.currentTarget&&setEventModal(null)}>
          <div className="gr-modal">
            <div className="gr-mtitle">📅 {eventModal==="new"?"Add Event":"Edit Event"}</div>
