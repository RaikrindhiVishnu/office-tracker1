...
export default function GreetingsAdmin() {
  ...
  const saveAch = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...achForm, employeeName: achForm.employeeName.trim(), employeeEmail: achForm.employeeEmail.trim().toLowerCase(), title: achForm.title.trim(), description: achForm.description.trim(), updatedAt: serverTimestamp() };
  ...
  const saveJoin = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...joinForm, name: joinForm.name.trim(), email: joinForm.email.trim().toLowerCase(), department: joinForm.department.trim() || null, designation: joinForm.designation.trim() || null, updatedAt: serverTimestamp() };
  ...
  const saveEvent = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...eventForm, title: eventForm.title.trim(), description: eventForm.description.trim(), location: eventForm.location?.trim() || null, rsvpLink: eventForm.rsvpLink?.trim() || null, updatedAt: serverTimestamp() };
